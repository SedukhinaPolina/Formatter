 ...
 * ''' handler.
 */
public class CharQuoteHandler implements IHandler {
    /**
     * Handle '''.
     *
    ...
    public void handle(final char symbol, final CodeClearance codeClearance, final IWriter out) throws WriterException {
        out.write("\'");
        if (codeClearance.isComment()) {
            return;
        }
        if (codeClearance.isQuote()) {
            codeClearance.setQuote(false);
        } else {
            codeClearance.setQuote(true);
        }
